        ...
        <table className="w-full">
          ...
          <tbody>
            {loading && items.length === 0 ? (
              <tr><td colSpan={10} className="table-td text-center text-[#737373] py-8">Loading…</td></tr>
            ) : (
              items.map((r) => {
                ...
                return (
                  <tr key={r.barcode} className="table-row">
                    ...
                    <td className="table-td">
                      <div className="text-[13px] font-medium text-[#0A0A0A]">{r.item_name || "—"}</div>
                      <div className="text-[11px] text-[#737373]">{r.subcategory_name || r.counter_name || ""}</div>
                    </td>
